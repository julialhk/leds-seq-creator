...
def brightness(beats_per_cycle, beat_feel, energy = 1.0):
    t = timing.get_timing()
    ...
    if beat_feel == BeatFell.no_beat:
        ...
        return BrightnessAnimation(t, RepeatFloatFunc.from_timing(t, beats_per_cycle, LinearFloatFunc(1.0 - energy, 1.0)))
...
def hue_shift(beats_per_cycle, timing, beat_feel, energy = 1.0):
    ...
    if beat_feel == BeatFell.no_beat:
        ...
        return HueShiftAnimation(t, RepeatFloatFunc.from_timing(t, beats_per_cycle, LinearFloatFunc(energy * 0.5, 0.0)))
...
def snake(beats_per_cycle, beat_feel, energy = 1.0):
    t = timing.get_timing()
    ...
    if beat_feel == BeatFell.no_beat:
        return SnakeAnimation(t, head, ConstFloatFunc(tail_length), ConstBooleanFunc())
